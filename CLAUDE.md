...
`main` auto-deploys to a single EC2 instance via `.github/workflows/deploy.yml` (no staging environment) — it checks out via `git archive`/API instead of Marketplace actions, rebuilds the Docker image, runs `alembic upgrade head` on container start, and verifies the container stayed up. Production Postgres is Supabase, not a container. Secrets live in the GitHub `production` environment (`PRODUCTION_ENV` is the full `.env` contents) — see README for the exact variable list and where each secret comes from.
