...
def test_calendar_day_flags_diet_followed_and_medication_taken():
    report = build_report(diet_adherence=True, exercise_adherence=True, medication_adherence=True)

    day = PatientDashboardService(db=None)._build_calendar_day(date.today(), [report])

    assert day.diet_followed is True
    assert day.exercise_followed is True
    assert day.medication_taken is True


def test_calendar_day_does_not_flag_when_answered_no():
    report = build_report(diet_adherence=False, exercise_adherence=False, medication_adherence=False)
...
def test_calendar_day_does_not_flag_when_unanswered():
    report = build_report(diet_adherence=None, exercise_adherence=None, medication_adherence=None, completed=False)
...
def test_calendar_checkin_exposes_diet_and_medication_adherence():
    report = build_report(diet_adherence=True, exercise_adherence=True, medication_adherence=False)
